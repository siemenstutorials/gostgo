...
function check_root() {
  [[ $EUID != 0 ]] && echo -e "${Error} 没有ROOT权限，无法继续操作 ${Green_background_prefix}sudo su${Font_color_suffix} 命令获ROOT权限" && exit 1
}
...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    wget --no-check-certificate https://raw.githubusercontent.com/siemenstutorials/gostgo/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && wget --no-check-certificate https://raw.githubusercontent.com/siemenstutorials/gostgo/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi
...
update_sh() {
  ol_version=$(curl -L -s https://raw.githubusercontent.com/siemenstutorials/gostgo/master/gost.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [[ "$shell_version" != "$ol_version" ]]; then
    echo -e "存在新版本，是否更新；不更新直接回车 [Y/N]?"
    read -r update_confirm
    case $update_confirm in
    [yY][eE][sS] | [yY])
      wget -N --no-check-certificate https://raw.githubusercontent.com/siemenstutorials/gostgo/master/gost.sh
      echo -e "更新完成"
      exit 0
      ;;
    *) ;;
